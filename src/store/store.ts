  ...
  persist(
    (set, get) => ({
      ...
      signup: (name: string, email: string, _password: string) => {
        ...
        localStorage.setItem('smartups_user', JSON.stringify(newUser))
      },
      logout: () => {
      ...
      updateUser: (updates: Partial<User>) => {
        set((state) => {
          ...
          if (updatedUser) {
            // Also update localStorage so it persists across reloads
            const storedUser = localStorage.getItem('smartups_user')
            if (storedUser) {
              const user = JSON.parse(storedUser)
              localStorage.setItem('smartups_user', JSON.stringify({
                ...user,
